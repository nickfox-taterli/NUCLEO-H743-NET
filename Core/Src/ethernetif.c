...
static void low_level_init(struct netif *netif)
{
  ...
  xTaskCreate((TaskFunction_t)ethernetif_input, "EthIf", INTERFACE_THREAD_STACK_SIZE, netif, configMAX_PRIORITIES - 1, NULL);
...
static err_t low_level_output(struct netif *netif, struct pbuf *p)
{
  ...
  while(xSemaphoreTake(TxPktSemaphore, TIME_WAITING_FOR_INPUT)!=pdPASS)
  {
  }
...
static void ethernetif_input( void const * argument )
{
  ...
  for( ;; )
  {
    if (xSemaphoreTake( RxPktSemaphore, TIME_WAITING_FOR_INPUT)==pdPASS)
    {
      do
      {
